...
404 NOT FOUND LAB 팀은 아동학대 사례가 지속적으로 증가하는 현실 속에서 (2022년 44,531건 → 2024년 47,096건), 아이들이 자신의 감정을 표현하고 회복할 수 있도록 돕는 기술적 솔루션을 개발하고 있습니다.
...
마음의 책장은 AI 기술과 심리치료 원리를 결합하여, 각 아동의 상황에 맞는 치유 동화를 자동으로 생성하는 것을 목표로 합니다.
...
### 1. 프롬프트 최적화
- 아동의 나이, 학대 유형, 상황을 분석하여 맞춤형 동화 생성 예정
- 전문적인 동화 작가의 작법을 학습한 AI 모델 활용 계획

### 2. PTSD 단어 배제
- 트라우마를 유발할 수 있는 직접적인 표현 자동 필터링 구현 중
- 은유와 상징을 통한 부드러운 접근 방식 적용 예정

### 3. 사용자 음성 기반 입력 (STT)
- 아이가 직접 자신의 이야기를 음성으로 녹음하는 기능 개발 중
- n8n 워크플로우를 통한 자동 음성-텍스트 변환 시스템 구축 중
- 음성 데이터 기반 맞춤형 동화 생성 파이프라인 설계 중

### 4. 다중 입력 방식
- **폼 기반**: 보호자가 아동 정보를 입력하여 동화 생성 (프로토타입 완성)
- **음성 기반**: 아동이 직접 음성으로 자신의 이야기를 전달 (개발 중)
...
예선 기획서를 고도화하여 MVP(Minimum Viable Product)를 개발하고 있습니다.
현재 음성 입력 인터페이스와 기본 워크플로우 구축을 진행 중이며, 본선까지 핵심 기능을 완성할 예정입니다.

### 프로토타입 테스트

**개발 중인 음성 입력 인터페이스 체험**
...
3. 인증 코드 입력 (테스트용)
   ...
   - `maum2025`
   - `404notfound`
4. 음성 녹음 UI 확인 (워크플로우 연동 개발 중)

**참고**: 현재는 UI 프로토타입 단계이며, 실제 동화 생성 기능은 본선까지 완성 예정입니다.
